...
func (m *MarketplaceHandler) Handle(db *gorm.DB, msg sdk.Msg, events ...abciTypes.Event) error {
	...
	case nft.MsgMintNFT:
		...
		if err := m.uriSender.Publish(value.ID, value.TokenURI, value.Recipient.String(), 0); err != nil {
	...
	case nft.MsgEditNFTMetadata:
		...
		if err := m.uriSender.Publish(value.ID, value.TokenURI, value.Sender.String(), 0); err != nil {
			return fmt.Errorf("failed to send message to RabbitMQ: %v", err)
		}
